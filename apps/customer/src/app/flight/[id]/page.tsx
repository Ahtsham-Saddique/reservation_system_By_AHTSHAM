...
export default function FlightDetail() {
  ...
  const handleReserve = () => {
    ...
    if (!flight) return;
    addToCart({
      id: Date.now().toString(), itemModel: 'flight', itemId: flight._id, name: `${flight.origin} to ${flight.destination}`,
      price: flight.price, quantity: passengers, selectedDate: travelDate, selectedTime: flightTimeWindow,
      details: { travelDate, flightTimeWindow, airline: flight.airline, flightNumber: flight.flightNumber, origin: flight.origin, destination: flight.destination, passengers },
  ...
  useEffect(() => {
    const fetchFlight = async () => {
      try {
        ...
        if (data.success) setFlight(data.data);
      } catch (error) { console.error("Failed to fetch flight", error); } finally { setLoading(false); }
    };
    if (id) fetchFlight();
  }, [id]);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div className="text-xl font-bold text-gray-800">Preparing for takeoff...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white pb-32">
      <div className="relative h-[50vh] min-h-[400px] w-full bg-gray-100">
        <img src="https://images.unsplash.com/photo-1436491865332-7a61a109cc05?auto=format&fit=crop&w=1920&q=80" alt="Flight" className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-gray-900 tracking-tight leading-none mb-2">{flight.origin} to {flight.destination}</h1>
        </div>
      </div>
                ...
                <div className="p-8">
                  <div className="space-y-4 mb-6 bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100">
                    ...
                    <div>
                      ...
                      <select value={flightTimeWindow} onChange={(e) => setFlightTimeWindow(e.target.value)} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-800 focus:ring-2 focus:ring-emerald-400 focus:outline-none">
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-emerald-100/60 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={() => setPassengers(p => Math.max(1, p - 1))} className="w-6 h-6 rounded-lg bg-gray-200 hover:bg-gray-300 font-black text-xs flex items-center justify-center">-</button>
                        <span className="font-black text-sm text-gray-900 w-4 text-center">{passengers}</span>
                        <button type="button" onClick={() => setPassengers(p => Math.min(8, p + 1))} className="w-6 h-6 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs flex items-center justify-center">+</button>
                      </div>
                    </div>
                  </div>
