...
export default function TourDetail() {
  ...
  const [tourDate, setTourDate] = useState(new Date(Date.now() + 86400000).toISOString().split('T')[0]);
  ...
  const handleReserve = () => {
    ...
    if (!tour) return;
    addToCart({
      id: Date.now().toString(), itemModel: 'tour', itemId: tour._id, name: tour.title,
      price: tour.price, quantity: participants, selectedDate: tourDate, selectedTime: tourTimeSlot,
      details: { tourDate, tourTimeSlot, durationDays: tour.durationDays, pickupPoint: tour.pickupPoint, participants },
  ...
  useEffect(() => {
    const fetchTour = async () => {
      ...
      } catch (error) { console.error("Failed to fetch tour", error); } finally { setLoading(false); }
    };
    if (id) fetchTour();
  }, [id, API]);
  ...
  useEffect(() => {
    ...
    const checkEligibility = async () => {
      try {
        ...
        const res = await fetch(`${API}/reviews/can-review?itemId=${id}&itemType=tour`, { headers: { Authorization: `Bearer ${token}` } });
        const data = await res.json();
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div className="text-xl font-bold text-gray-800">Preparing your adventure...</div>
        </div>
      </div>
    );
  }
  ...
  const avgRating = reviews.length > 0 ? (reviews.reduce((s, r) => s + r.rating, 0) / reviews.length).toFixed(1) : tour.rating || 0;

  return (
    <div className="min-h-screen bg-white pb-32">
      <div className="relative h-[50vh] min-h-[400px] w-full bg-gray-100">
        <img src={tour.images && tour.images.length > 0 ? tour.images[0] : "https://images.unsplash.com/photo-1510312305653-8ed496efae75?auto=format&fit=crop&w=1920&q=80"} alt={tour.title} className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-gray-900 tracking-tight leading-none mb-2">{tour.title}</h1>
        </div>
      </div>
          ...
          <div className="lg:col-span-7 xl:col-span-8 space-y-16">
            ...
            <section>
              <div className="flex items-center justify-between mb-10">
                ...
                <button className="text-sm font-bold text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-4 py-2 rounded-xl transition-colors">Download PDF</button>
              </div>
            ...
            <section>
              <EcoMap locationName={tour.title} address={tour.pickupPoint} city={tour.pickupPoint} category="Nature Eco Tour" />
            </section>
            ...
            <section id="reviews">
              ...
              {reviews.length === 0 ? (
                <div className="text-center py-12 text-gray-400 font-medium">No reviews yet. Be the first to share your adventure!</div>
              ) : (
                ...
                <div className="p-8">
                  ...
                  <div className="space-y-4 mb-6 bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100">
                    ...
                    <div>
                      ...
                      <input type="date" value={tourDate} min={new Date().toISOString().split('T')[0]} onChange={(e) => setTourDate(e.target.value)} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-900 focus:ring-2 focus:ring-emerald-400 focus:outline-none" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Time Slot / Session</label>
                      <select value={tourTimeSlot} onChange={(e) => setTourTimeSlot(e.target.value)} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-800 focus:ring-2 focus:ring-emerald-400 focus:outline-none">
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-emerald-100/60 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        <button type="button" onClick={() => setParticipants(p => Math.max(1, p - 1))} className="w-6 h-6 rounded-lg bg-gray-200 hover:bg-gray-300 font-black text-xs flex items-center justify-center">-</button>
                        <span className="font-black text-sm text-gray-900 w-4 text-center">{participants}</span>
                        <button type="button" onClick={() => setParticipants(p => Math.min(tour.maxGroupSize || 20, p + 1))} className="w-6 h-6 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs flex items-center justify-center">+</button>
                      </div>
                    </div>
                  </div>
