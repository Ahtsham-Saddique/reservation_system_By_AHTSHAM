...
export default function MyBookings() {
  ...
  const handleCancelBooking = async (id: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) setBookings(bookings.map(b => b._id === id ? { ...b, status: 'cancelled' } : b));
    ...
    <div className="min-h-screen bg-white pb-24 pt-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-gray-500 font-medium mt-1">Manage reservations and view travel details.</p>
        </div>
        ...
        {bookings.length === 0 ? (
          <div className="text-center py-24 bg-white rounded-3xl border border-gray-100 shadow-sm p-8 max-w-lg mx-auto">
            ...
            <h3 className="text-2xl font-black text-gray-900 mb-2">No reservations yet</h3>
            <p className="text-gray-500 font-medium mb-8">Your bookings will appear here after you make a reservation.</p>
            <Link href="/" className="inline-flex items-center gap-2 px-6 py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-2xl shadow-lg shadow-emerald-600/20 transition-all hover:-translate-y-0.5">
          ...
          <div className="space-y-6">
            {bookings.map((booking) => (
              <div key={booking._id} className="bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm">
                <div className="p-6 sm:p-8 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
                  <div className="flex-1 space-y-4">
                    <div className="flex flex-wrap items-center gap-3">
                      <span className={`px-3 py-1 rounded-full text-xs font-black uppercase ${statusStyles[booking.status] || 'bg-gray-100 text-gray-800'}`}>
                  ...
                  <div className="flex flex-col sm:flex-row md:flex-col items-start sm:items-center md:items-end gap-3 w-full md:w-auto pt-4 md:pt-0 border-t md:border-t-0 border-gray-100">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      <button onClick={() => setSelectedVoucher(booking)} className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs transition-colors flex items-center gap-1.5 shadow-md shadow-emerald-600/20">
                        <span>📄</span> Official Voucher & PDF
                      </button>
                      {(booking.status === 'confirmed' || booking.status === 'pending') && (
                        <button onClick={() => handleCancelBooking(booking._id)} disabled={cancelLoading === booking._id} className="px-4 py-2.5 bg-white border border-red-200 text-red-600 hover:bg-red-50 font-bold rounded-xl text-xs transition-colors disabled:opacity-50">
      ...
      {selectedVoucher && (
        <div className="fixed inset-0 bg-gray-950/70 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[94vh] overflow-y-auto shadow-2xl border border-gray-100 p-5 sm:p-6">
            <div className="flex items-center justify-between pb-3 mb-4 border-b border-gray-100 print:hidden">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => window.print()} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-md flex items-center gap-1.5 transition-all">
                  <span>🖨️</span> Print / Save PDF
                </button>
                <button onClick={() => setSelectedVoucher(null)} className="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-sm font-bold text-gray-500">
                  ✕
                </button>
              </div>
            </div>

            <div className="border-2 border-dashed border-emerald-300 rounded-3xl p-5 sm:p-6 bg-gradient-to-b from-[#f7fbf9] to-white space-y-4">
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-emerald-600 rounded-lg flex items-center justify-center text-white text-base font-black shadow-sm">🍃</div>
                ...
                <div className="text-right">
                  <span className={`px-3 py-0.5 rounded-full text-[11px] font-black uppercase tracking-wider ${selectedVoucher.status === 'confirmed' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-blue-100 text-blue-800'}`}>
                    {selectedVoucher.status === 'confirmed' ? 'CONFIRMED • PAID' : selectedVoucher.status.toUpperCase()}
                  </span>
                  <p className="font-mono text-xs font-black text-gray-900 mt-1">Order Ref: #{selectedVoucher._id.slice(-8).toUpperCase()}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 p-3.5 bg-white rounded-2xl border border-gray-100 text-xs shadow-sm">
                ...
                <div>
                  ...
                  <p className="font-black text-gray-900 text-xs mt-0.5">{new Date(selectedVoucher.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                  <p className="text-gray-500 text-[10px] font-medium">{new Date(selectedVoucher.createdAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}</p>
                </div>
                <div>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-dashed border-gray-200">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-800">
                    <span>🌱</span> 100% Carbon Neutral Certified
                  </div>
            ...
            <div className="mt-4 flex justify-end print:hidden">
              <button onClick={() => setSelectedVoucher(null)} className="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-xl text-xs transition-colors">
                Close Voucher
              </button>
            </div>
          </div>
