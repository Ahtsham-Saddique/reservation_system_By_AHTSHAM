...
function StarPicker({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  ...
  return (
    <span className="inline-flex gap-1 text-3xl cursor-pointer select-none">
      {[1, 2, 3, 4, 5].map((s) => (
        <span key={s} onMouseEnter={() => setHovered(s)} onMouseLeave={() => setHovered(0)} onClick={() => onChange(s)} className={(hovered || value) >= s ? 'text-amber-400' : 'text-gray-300'}>★</span>
      ))}
    </span>
  );
}
...
export default function HotelDetail() {
  ...
  const nights = Math.max(1, Math.round((new Date(checkOutDate).getTime() - new Date(checkInDate).getTime()) / (1000 * 60 * 60 * 24)) || 1);
  ...
  const handleReserve = () => {
    ...
    addToCart({
      id: Date.now().toString(), itemModel: 'hotel', itemId: hotel._id, name: `${hotel.name} (${currentRoom.roomType})`,
      price: totalPrice, quantity: 1, selectedDate: `${checkInDate} to ${checkOutDate} (${nights} night${nights > 1 ? 's' : ''})`,
      selectedTime: checkInTime, details: { checkInDate, checkOutDate, nights, checkInTime, guests: guestsCount, roomType: currentRoom.roomType, roomPrice: currentRoom.price },
  ...
  useEffect(() => {
    const fetchHotel = async () => {
      ...
      } catch (error) { console.error("Failed to fetch hotel", error); } finally { setLoading(false); }
    };
    if (id) fetchHotel();
  }, [id, API]);
  ...
  useEffect(() => {
    ...
    const checkEligibility = async () => {
      try {
        ...
        const res = await fetch(`${API}/reviews/can-review?itemId=${id}&itemType=hotel`, { headers: { Authorization: `Bearer ${token}` } });
        const data = await res.json();
        if (data.success) { setCanReview(data.canReview); if (!data.canReview && data.reason === 'Already reviewed.') setAlreadyReviewed(true); }
      } catch (e) { /* silent */ }
    };
    checkEligibility();
  }, [user, id, API]);
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) { setReviews([data.data, ...reviews]); setCanReview(false); setAlreadyReviewed(true); setReviewComment(''); setReviewMsg('success'); }
      else { setReviewMsg(`error:${data.message}`); }
    } catch (e) { setReviewMsg('error:Something went wrong. Please try again.'); } finally { setReviewSubmitting(false); }
  };
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div className="text-xl font-bold text-gray-800">Preparing your stay...</div>
        </div>
      </div>
    );
  }
  ...
  const avgRating = reviews.length > 0 ? (reviews.reduce((s, r) => s + r.rating, 0) / reviews.length).toFixed(1) : hotel.rating || 0;

  return (
    <div className="min-h-screen bg-white pb-32">
      <div className="relative h-[50vh] min-h-[400px] w-full bg-gray-100">
        <img src={hotel.images && hotel.images.length > 0 ? hotel.images[0] : "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=1920&q=80"} alt={hotel.name} className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 z-10">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-gray-900 tracking-tight leading-none mb-2">{hotel.name}</h1>
          <p className="text-lg font-bold text-gray-800">📍 {hotel.location?.city}, {hotel.location?.country}</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16">
        <div className="lg:grid lg:grid-cols-12 lg:gap-16 items-start">
          <div className="lg:col-span-7 xl:col-span-8 space-y-16">
              ...
              {hotel.policies && hotel.policies.length > 0 && (
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                  ...
                  <ul className="list-disc list-inside text-gray-700 space-y-2 font-medium">
                    {hotel.policies.map((policy: string, i: number) => <li key={i}>{policy}</li>)}
                  </ul>
                </div>
              )}
            </section>
            ...
            <section>
              <EcoMap locationName={hotel.name} address={hotel.location?.address} city={hotel.location?.city} category="Eco Hotel & Stay" />
            </section>
            ...
            <section id="reviews">
              ...
              {!user && (
                <div className="bg-gray-50 border border-gray-100 rounded-3xl p-6 mb-8 text-center">
                  <p className="text-gray-500 font-medium"><a href="/login" className="text-emerald-600 font-bold hover:underline">Sign in</a> to leave a review.</p>
                </div>
              )}
              ...
              {user && canReview && (
                <div className="bg-white border border-emerald-100 rounded-3xl p-8 mb-8 shadow-sm">
                  ...
                  {reviewMsg === 'success' && <div className="mb-4 p-3 bg-emerald-50 border border-emerald-100 rounded-xl text-emerald-700 font-bold text-sm">✅ Review submitted! Thank you.</div>}
                  {reviewMsg.startsWith('error:') && <div className="mb-4 p-3 bg-red-50 border border-red-100 rounded-xl text-red-600 font-bold text-sm">❌ {reviewMsg.replace('error:', '')}</div>}
                  <form onSubmit={handleSubmitReview} className="space-y-5">
                    ...
                    <div>
                      ...
                      <textarea required rows={4} value={reviewComment} onChange={(e) => setReviewComment(e.target.value)} placeholder="Describe your experience — what did you love? What could be better?" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-400 focus:outline-none resize-none font-medium text-gray-800" />
                    </div>
                    <button type="submit" disabled={reviewSubmitting} className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-xl shadow-sm transition-all disabled:opacity-60">
              ...
              {reviews.length === 0 ? (
                <div className="text-center py-12 text-gray-400 font-medium">No reviews yet. Be the first to share your experience!</div>
              ) : (
                    ...
                    <div key={review._id} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-black text-lg">{review.user?.name?.[0]?.toUpperCase() || '?'}</div>
                          <div>
                ...
                <div className="p-8">
                  <div className="space-y-4 mb-6 bg-emerald-50/50 p-4 rounded-2xl border border-emerald-100">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="date" value={checkInDate} min={new Date().toISOString().split('T')[0]} onChange={(e) => setCheckInDate(e.target.value)} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-900 focus:ring-2 focus:ring-emerald-400 focus:outline-none" />
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Check-out Date</label>
                        <input type="date" value={checkOutDate} min={checkInDate || new Date().toISOString().split('T')[0]} onChange={(e) => setCheckOutDate(e.target.value)} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-900 focus:ring-2 focus:ring-emerald-400 focus:outline-none" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Estimated Arrival Time</label>
                      <select value={checkInTime} onChange={(e) => setCheckInTime(e.target.value)} className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-800 focus:ring-2 focus:ring-emerald-400 focus:outline-none">
                  ...
                  <div className="space-y-3 mb-6">
                    ...
                    {hotel.rooms?.map((room: any, i: number) => {
                      ...
                      return (
                        <button key={i} onClick={() => setSelectedRoomIndex(i)} type="button" className={`w-full text-left flex justify-between items-center p-3.5 rounded-2xl transition-all cursor-pointer ${isSelected ? 'border-2 border-emerald-500 bg-emerald-50/80 shadow-md shadow-emerald-600/10 ring-2 ring-emerald-500/20' : 'border border-gray-200/80 bg-white hover:border-gray-300 hover:bg-gray-50/50'}`}>
                          <div className="flex items-center gap-3">
                            <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black transition-all ${isSelected ? 'bg-emerald-600 text-white shadow-sm' : 'border-2 border-gray-300 bg-white'}`}>
                              {isSelected ? '✓' : ''}
                            </div>
                            <div>
                              <div className={`font-bold text-sm leading-tight ${isSelected ? 'text-emerald-950 font-black' : 'text-gray-900'}`}>{room.roomType}</div>
                              <div className="text-xs text-gray-500 font-medium mt-0.5">Capacity: {room.capacity} Guests</div>
                            </div>
                          </div>
                          <div className="text-right pl-2">
                            <div className={`text-sm font-black ${isSelected ? 'text-emerald-700' : 'text-gray-900'}`}>${room.price}/nt</div>
                            {isSelected && <span className="text-[10px] font-bold text-emerald-600">Selected</span>}
                          </div>
                        </button>
                      );
                    })}
                  ...
                  {!user ? (
                    <Link href="/login" className="w-full py-4 text-white font-black rounded-2xl shadow-xl transition-all hover:-translate-y-1 active:scale-95 text-base sm:text-lg bg-gray-900 hover:bg-gray-800 shadow-gray-900/20 text-center block">
                      Login to Reserve
                    </Link>
                  ) : (
                    <button onClick={handleReserve} disabled={added} className={`w-full py-4 text-white font-black rounded-2xl shadow-xl transition-all hover:-translate-y-1 active:scale-95 text-base sm:text-lg ${added ? 'bg-gray-900 shadow-gray-900/20' : 'bg-emerald-600 hover:bg-emerald-500 shadow-emerald-600/20'}`}>
